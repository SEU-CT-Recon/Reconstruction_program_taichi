...
class Mgfpj:
    def MainFunction(self):
        ...
        self.GenerateAngleArray(self.view_num,self.start_angle,self.scan_angle,self.array_angle_taichi)
        self.GenerateDectPixPosArray(self.dect_elem_count_vertical, self.dect_elem_height,self.dect_offset_vertical,self.array_v_taichi)
    ...
    def __init__(self,config_dict):
        ...
        if 'StartAngle' in config_dict:
            self.start_angle = config_dict['StartAngle'] / 180 * PI
        else:
            self.start_angle = 0
        if 'TotalScanAngle' in config_dict:
            self.scan_angle = config_dict['TotalScanAngle'] / 180 * PI
        else:
            self.scan_angle = 2* PI

        if abs(self.scan_angle%(2*PI)) < (0.01 / 180 * PI):
            print('--Full scan, scan Angle = %.1f degrees' % (self.scan_angle / PI * 180))
            # self.return_information += '--Full scan, scan Angle = %.1f degrees\n' % (self.total_scan_angle / PI * 180)
        else:
            print('--Short scan, scan Angle = %.1f degrees' % (self.scan_angle / PI * 180))
        ...
        if self.cone_beam:
            ...
            self.img_voxel_height = 0
            self.dect_elem_height = 0
            self.dect_offset_vertical = 0
        ...
        self.array_u_taichi = ti.field(dtype=ti.f32,shape=self.dect_elem_count_horizontal*self.oversample_size)
        self.array_v_taichi = ti.field(dtype=ti.f32,shape=self.dect_elem_count_horizontal)
        self.img_image_taichi = ti.field(dtype=ti.f32, shape=(self.img_dim_z,self.img_dim, self.img_dim))
        #for sgm in gpu ram, we initialize 2D buffer; since gpu ram is limited
        self.img_sgm_large_taichi = ti.field(dtype=ti.f32, shape=(1, self.view_num, self.dect_elem_count_horizontal*self.oversample_size),order='ijk',needs_dual = True)
    ...
    def GenerateDectPixPosArray(self,dect_elem_count_horizontal:ti.i32,dect_elem_width:ti.f32,dect_offset_horizontal:ti.f32,array_u_taichi:ti.template()):
        for i in ti.ndrange(dect_elem_count_horizontal):
            array_u_taichi[i] = (i - (dect_elem_count_horizontal - 1) / 2.0) * dect_elem_width + dect_offset_horizontal
    ...
    def ForwardProjectionBilinear(self,img_image_taichi:ti.template(),img_sgm_large_taichi:ti.template(),\
        ...
        x_rot = y_rot = z_rot = 0.0  # initialize position of the voxel for current step after rotation
        x_idx = y_idx = z_idx = 0 # initialize index of the voxel for current step
        x_weight = y_weight = z_weight = 0.0 # weighting factor of the voxel for linear interpolation
        ...
        z_0 = -( img_dim_z - 1.0) / 2.0 * img_voxel_height + img_center_z #by default, the first slice corresponds to the bottom of the image object
            

        dect_elem_pos_x = dect_elem_pos_y = dect_elem_pos_z = 0.0; #initialize coordinate for the detector element
        source_dect_elem_dis = 0.0 #initialize detector element to source distance
        unit_vec_lambda_x = unit_vec_lambda_y = unit_vec_lambda_z = 0.0 #initialize detector element to source unit vector
        l_min = sid - (2 * img_dimension **2 + image_dimension_z **2)**0.5 / 2.0 #lower range for the line integral
        l_max = sid + (2 * img_dimension **2 + image_dimension_z **2)**0.5 / 2.0 #upper range for the line integral
        voxel_diagonal_size = ( 2*(img_pix_size ** 2) + (img_voxel_height ** 2))**0.5
        ...
        if self.helican_scan:
            ...
            z_dis_per_view = helical_pitch * (num_laps / view_num) * (abs(array_v_taichi[1] - array_v_taichi[0]) * dect_elem_count_vertical) / (sdd / sid)
        
        #count of steps
        count_steps = int(ti.floor((l_max - l_min)/(fpj_step_size * voxel_diagonal_size)))
        
        #for u_idx, v_idx in ti.ndrange(dect_elem_count_horizontal_oversamplesize,dect_elem_count_vertical):
        for u_idx,angle_idx in ti.ndrange(dect_elem_count_horizontal_oversamplesize, view_num):
            ...
            if self.curved_dect:
                ...
                dect_elem_pos_y = -sdd * ti.sin(gamma_prime) # positive u direction is - y
            else:
                dect_elem_pos_x = - (sdd - sid)
                dect_elem_pos_y = - array_u_taichi[u_idx] # positive u direction is - y
                
            dect_elem_pos_z = - array_v_taichi[v_idx] + z_dis_per_view * angle_idx # positive v direction is - z
            source_pos_z = z_dis_per_view * angle_idx #assume that the source and the detector moves upward for a helical scan (pitch>0)
            
            source_dect_elem_dis = ((dect_elem_pos_x - source_pos_x)**2 + (dect_elem_pos_y - source_pos_y)**2 + (dect_elem_pos_z - source_pos_z)**2) ** 0.5
            ...
            for step_idx in ti.ndrange(count_steps):
                x = source_pos_x + unit_vec_lambda_x * (step_idx * fpj_step_size * voxel_diagonal_size + l_min)
                y = source_pos_y + unit_vec_lambda_y * (step_idx * fpj_step_size * voxel_diagonal_size + l_min)
                z = source_pos_z + unit_vec_lambda_z * (step_idx * fpj_step_size * voxel_diagonal_size + l_min) 
                
                x_rot = x * ti.cos(array_angle_taichi[angle_idx]) - y * ti.sin(array_angle_taichi[angle_idx])
                y_rot = y * ti.cos(array_angle_taichi[angle_idx]) + x * ti.sin(array_angle_taichi[angle_idx])
                z_rot = z
                ...
                if x_idx >=0 and x_idx+1 < img_dim and y_idx >=0 and y_idx+1 < img_dim :
                    x_weight = ( x_rot - ( x_idx * img_pix_size + x_0 )) / img_pix_size
                    y_weight = ( y_rot - ( y_idx * (- img_pix_size) + y_0 )) / (- img_pix_size)
                                        
                    if self.cone_beam: 
                        z_idx = int(ti.floor( (z_rot - z_0) / img_voxel_height))
                        
                        if z_idx >= 0 and z_idx + 1 < img_dim_z:
                            z_weight = ( z_rot - ( z_idx * img_voxel_height + z_0 )) / img_voxel_height
                            sgm_val_lowerslice = (1.0 - x_weight) * (1.0 - y_weight) * img_image_taichi[z_idx,y_idx,x_idx]\
                                ...
                                + x_weight * y_weight * img_image_taichi[z_idx,y_idx+1,x_idx + 1]
                            sgm_val_upperslice = (1.0 - x_weight) * (1.0 - y_weight) * img_image_taichi[z_idx + 1,y_idx,x_idx]\
                                ...
                                + x_weight * y_weight * img_image_taichi[z_idx+ 1,y_idx+1,x_idx + 1]
                            temp_sgm_val += ((1.0 - z_weight) * sgm_val_lowerslice + z_weight * sgm_val_upperslice) * fpj_step_size * voxel_diagonal_size
                    else:
                        z_idx = v_idx
    ...
    def ReadImage(self,file):
        ...
        if self.output_file == file:
            ...
            self.img_image = self.img_image.reshape(self.img_dim_z,self.img_dim, self.img_dim)
            if self.convert_to_HU:
                self.img_image = (self.img_image + 1000.0) / 1000.0 * self.water_mu
    ...
    def SaveSinogram(self):
        if self.output_file_form == 'sinogram':
            ...
            self.img_sgm = self.img_sgm.transpose([1,0,2])
            imwriteRaw(self.img_sgm,self.output_path,dtype=np.float32)
