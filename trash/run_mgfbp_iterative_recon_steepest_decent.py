# -*- coding: utf-8 -*-
"""
Created on Tue Jul 23 13:21:10 2024
...
class Mgfbp_ir(Mgfbp):
    def __init__(self,config_dict):
        ...
        self.img_x0 = np.zeros((self.img_dim_z,self.img_dim, self.img_dim),dtype=np.float32)
        self.img_Ax0 = np.zeros((self.img_dim_z,self.img_dim, self.img_dim),dtype=np.float32)
        self.img_r0 = np.zeros((self.img_dim_z,self.img_dim, self.img_dim),dtype=np.float32)
        self.img_PTb = np.zeros((self.img_dim_z,self.img_dim, self.img_dim),dtype=np.float32)
        
        self.img_fpj_x0 = np.zeros((self.dect_elem_count_vertical, self.view_num,
                                self.dect_elem_count_horizontal), dtype=np.float32)
        
        self.img_fpj_r0 = np.zeros((self.dect_elem_count_vertical, self.view_num,
                                self.dect_elem_count_horizontal), dtype=np.float32)
        
        self.img_Ar0 = np.zeros((self.img_dim_z,self.img_dim, self.img_dim),dtype=np.float32)
            ...
            if re.match(self.input_files_pattern, file):#match the file pattern
                if self.ReadSinogram(file):
                    ...
                    self.BackProjectionPixelDriven(self.dect_elem_count_vertical_actual, self.img_dim, self.dect_elem_count_horizontal, \
                                    ...
                                    self.array_angle_taichi, self.img_rot,self.img_sgm,self.img_PTb,\
                    ...
                    for idx in range(100):
                        print('%d\n' %idx)
                        self.ForwardProjectionBilinear(self.img_x0, self.img_fpj_x0, self.array_u_taichi,
                        ...
                        self.BackProjectionPixelDriven(self.dect_elem_count_vertical_actual, self.img_dim, self.dect_elem_count_horizontal, \
                                        self.view_num, self.dect_elem_width,self.img_pix_size, self.source_isocenter_dis, self.source_dect_dis,self.total_scan_angle,\
                                        self.array_angle_taichi, self.img_rot,self.img_fpj_x0,self.img_Ax0,\
                                        self.array_u_taichi,self.short_scan,self.cone_beam,self.dect_elem_height,\
                                            self.array_v_taichi,self.img_dim_z,self.img_voxel_height,\
                                                self.img_center_x,self.img_center_y,self.img_center_z,self.curved_dect,\
                                                    self.bool_apply_pmatrix,self.array_pmatrix_taichi, self.recon_view_mode)
                        
                        self.img_r0 = self.img_Ax0 - self.img_PTb
                        
                        self.ForwardProjectionBilinear(self.img_r0, self.img_fpj_r0, self.array_u_taichi,
                                                       self.array_v_taichi, self.array_angle_taichi, self.img_dim, self.img_dim_z,
                                                       self.dect_elem_count_horizontal,
                                                       self.dect_elem_count_vertical, self.view_num, self.img_pix_size, self.img_voxel_height,
                                                       self.source_isocenter_dis, self.source_dect_dis, self.cone_beam,
                        ...
                        self.BackProjectionPixelDriven(self.dect_elem_count_vertical_actual, self.img_dim, self.dect_elem_count_horizontal, \
                                        self.view_num, self.dect_elem_width,self.img_pix_size, self.source_isocenter_dis, self.source_dect_dis,self.total_scan_angle,\
                                        self.array_angle_taichi, self.img_rot,self.img_fpj_r0,self.img_Ar0,\
                                        self.array_u_taichi,self.short_scan,self.cone_beam,self.dect_elem_height,\
                                            self.array_v_taichi,self.img_dim_z,self.img_voxel_height,\
                                                self.img_center_x,self.img_center_y,self.img_center_z,self.curved_dect,\
                                                    self.bool_apply_pmatrix,self.array_pmatrix_taichi, self.recon_view_mode)
                            
                        alpha = - np.sum(np.multiply(self.img_r0,self.img_r0)) /np.sum(np.multiply(self.img_r0,self.img_Ar0)) 
                        self.img_x0 = self.img_x0 + np.multiply(alpha,self.img_r0) 
