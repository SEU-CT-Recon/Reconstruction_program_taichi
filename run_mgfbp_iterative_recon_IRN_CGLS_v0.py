...
class Mgfbp_ir(Mgfpj_v3):
    def __init__(self,config_dict):
        ...
        temp_array = self.array_img_center_z_taichi.to_numpy()
        temp_array = temp_array-0
        self.array_img_center_z_taichi.from_numpy(temp_array)
                ...
                if self.ReadSinogram(file):
                    ...
                    for irn_iter_idx in range(self.num_irn_iter):
                        ...
                        for iter_idx in range(self.num_iter):
                            ...
                            if iter_idx%1==0:
                                if self.convert_to_HU:
                                    ...
                                    plt.imshow((self.img_x[int(round(self.img_dim_z/2)),:,:]/ self.water_mu - 1)*1000,cmap = 'gray',vmin = -50, vmax = 100)
                        ...
                        if self.output_file_format == 'tif' or self.output_file_format == 'tiff':
                            imwriteTiff(self.img_x, self.output_path,dtype=np.float32)
