...
def run_mgfbp_huashi(file_path):
    ...
    fbp = Mgfbp_huashi(config_dict) #将config_dict数据以字典的形式送入对象
...
class Mgfbp_huashi(Mgfbp):
                                    ...
                                    self.array_u_taichi,self.short_scan,self.cone_beam,self.dect_elem_height,\
                                        self.array_v_taichi,self.img_dim_z,self.img_voxel_height,\
                                            self.img_center_x,self.img_center_y,self.img_center_z,self.curved_dect,\
                                                self.bool_apply_pmatrix,self.array_pmatrix_taichi, self.recon_view_mode, self.img_rot_y, self.det_rot)
    ...
    def __init__(self,config_dict):
        super(Mgfbp_huashi,self).__init__(config_dict) 
                                          ...
                                          array_u_taichi:ti.template(), short_scan:ti.i32,cone_beam:ti.i32,dect_elem_height:ti.f32,\
                                              array_v_taichi:ti.template(),img_dim_z:ti.i32,img_voxel_height:ti.f32, \
                                                  img_center_x:ti.f32,img_center_y:ti.f32,img_center_z:ti.f32,curved_dect:ti.i32,\
                                                      bool_apply_pmatrix:ti.i32, array_pmatrix_taichi:ti.template(), recon_view_mode: ti.i32, img_rot_y: ti.f32, det_rot:ti.f32):
            ...
            for i_z in ti.ndrange(img_dim_z):
                ...
                for j in ti.ndrange(view_num):
                    ...
                    if cone_beam == True:
                        ...
                        if temp_v_idx_floor < -1 or temp_v_idx_floor + 1 > dect_elem_count_vertical_actual: #mark
                            img_recon_taichi[i_z, i_y, i_x] += 0
                            #break
                        else:
                            ratio_v = pix_proj_to_dect_v_idx - temp_v_idx_floor
                            if temp_v_idx_floor == -1: #huashi
                                part_0 = 0
                                part_1 = img_sgm_filtered_taichi[0,j,temp_u_idx_floor] * (1 - ratio_u) +\
                                      img_sgm_filtered_taichi[0,j,temp_u_idx_floor + 1] * ratio_u
                                img_recon_taichi[i_z, i_y, i_x] += (source_isocenter_dis * distance_weight) * \
                                    ((1 - ratio_v) * part_0 + ratio_v * part_1) * delta_angle * div_factor
                            elif temp_v_idx_floor == dect_elem_count_vertical_actual-1: #huashi
                                part_1 = 0
                                part_0 = img_sgm_filtered_taichi[dect_elem_count_vertical_actual-1,j,temp_u_idx_floor] * (1 - ratio_u) + \
                                    img_sgm_filtered_taichi[dect_elem_count_vertical_actual-1,j,temp_u_idx_floor + 1] * ratio_u
                                img_recon_taichi[i_z, i_y, i_x] += (source_isocenter_dis * distance_weight) * \
                                    ((1 - ratio_v) * part_0 + ratio_v * part_1) * delta_angle * div_factor
                            else:
